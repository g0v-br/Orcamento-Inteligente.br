  ...
  export default {
    ...
    computed: {
      display() {
        ...
        (this.filtered == this.total) ? data = fetchData(false) : data = fetchData(this.filtered);
        
        
        text = data.text;
        if (!this.filtered) 
          text = text.replace("%s", total);
        else {
          rate = parseFloat(this.filtered) / parseFloat(this.total);

          if(rate < data.treshold)
            text = text.replace("%s", filtered).replace("%s", data.tresholdPrintTemplate);
          else {
...
function fetchData(filterActive) {
  ...
  object["treshold"] = parseFloat(bgoStore.anyValue(totalizer, ns.bgo("treshold")));
  object["tresholdPrintTemplate"] = bgoStore.anyValue(totalizer, ns.bgo("tresholdPrintTemplate"));
